...
const totalPrice = computed(() => {
    let total = 0
    itemList.value.forEach( item => {total += item.price * item.quantity })
return total
})
                                 ...
                                 <div class="flex flex-wrap -m-2">
                                   <div class="p-2 w-full">
                                     <div class="relative">
                                       ...
                                       <input type="date" id="name" name="date" v-model="form.date" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                       <div v-if="props.errors.date" class="text-red-500">{{ props.errors.date }}</div>
                                     </div>
                                   </div>
                                   <div class="p-2 w-full">
                                     <div class="relative">
                                       ...
                                       <div v-if="props.errors.customer_id" class="text-red-500">顧客は必ず選択してください</div>
                                     </div>
                                   </div>
                                   <table class="w-full table-auto w-full text-left whitespace-no-wrap">
                                    <thead>
                                        ...
                                        <tbody class="w-full">
                                            <tr v-for="item in itemList" class="w-full">
                                                <td class="w-1/8 border-t-2 border-b-2 border-gray-200 px-4 py-3">{{ item.id }}</td>
                                                <td class="w-3/8 border-t-2 border-b-2 border-gray-200 px-4 py-3">{{ item.name }}</td>
                                                <td class="w-1/8 border-t-2 border-b-2 border-gray-200 px-4 py-3">{{ item.price }}</td>
                                                <td class="w-2/8 border-t-2 border-b-2 border-gray-200 px-4 py-3">
                                                    <select class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" name="quantity" v-model="item.quantity">
                                    ...
                                    <div class="p-2 w-full">
                                       <button class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">購入</button>
                                    </div>
                                 </div>
                               </div>
                             </div>
